...
def experiment1(n_neighbors=5):
    ...
    """
    voice_features, _ = files_to_mfcc_features('an4/wav/an4_clstk')
    voice_labels = ["voice"] * voice_features.shape[0]

    white_noise_features, _ = files_to_mfcc_features('white_noise/train')
    ...
    white_noise_tests, _ = files_to_mfcc_features('white_noise/test')
    voice_tests, _ = files_to_mfcc_features('an4/wav/an4test_clstk')
...
def experiment2(n_neighbors=3):
    ...
    train_data, train_labels = files_to_mfcc_features('an4_pairwise/train_full')
    clf = train_knn(train_data, train_labels, n_neighbors)

    test_data, exp_labels = files_to_mfcc_features('an4_pairwise/test_full')
...
def experiment3(n_neighbors=3):
    ...
    train_data, train_labels = files_to_mfcc_features('an4_pairwise/train')
    clf = train_knn(train_data, train_labels, n_neighbors)

    test_data, exp_labels = files_to_mfcc_features('an4_pairwise/test')
